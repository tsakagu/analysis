#include "CaloTriggerEmulatorAnNeutral.h"
...
#include <cstdlib>
#include <iostream>
#include <numeric>  // for std:: XXX
#include <string>
#include <utility>

// constructor
CaloTriggerEmulatorAnNeutral::CaloTriggerEmulatorAnNeutral(const std::string &name)
  : SubsysReco(name)
{
...
bool CaloTriggerEmulatorAnNeutral::CheckChannelMasks(TriggerDefs::TriggerSumKey key)
{
...
bool CaloTriggerEmulatorAnNeutral::CheckFiberMasks(TriggerDefs::TriggerPrimKey key)
{
  return (std::find(m_masks_fiber.begin(), m_masks_fiber.end(), key) != m_masks_fiber.end());
}

void CaloTriggerEmulatorAnNeutral::LoadFiberMasks()
{
...
void CaloTriggerEmulatorAnNeutral::setTriggerType(const std::string &name)
{
  m_trigger = name;
...
void CaloTriggerEmulatorAnNeutral::setTriggerType(TriggerDefs::TriggerId triggerid)
{
  m_triggerid = triggerid;
}

// make file and histomanager (but nothing goes in the file at the moment)
int CaloTriggerEmulatorAnNeutral::Init(PHCompositeNode * /*topNode*/)
{
  return 0;
}

// at the beginning of the run
int CaloTriggerEmulatorAnNeutral::InitRun(PHCompositeNode *topNode)
{
...
int CaloTriggerEmulatorAnNeutral::Download_Calibrations()
{
  if (m_do_emcal)
  {
    // Permanently dead IB
    ...
    // Optionally ask bad towers in the emulator (of course not implemented in reality)
    if (m_use_hotMap)
    {
      m_fieldname_hotMap = "status";
      std::string calibName_hotMap = "CEMC_BadTowerMap";

      calibdir = CDBInterface::instance()->getUrl(calibName_hotMap);
      if (calibdir.empty())
      {
        std::cout << PHWHERE << "Hot map mask not found in CDB, not even in the default... " << std::endl;
        exit(1);
      }
      cdbttree_hotMap = new CDBTTree(calibdir);
    }
  }
...
int CaloTriggerEmulatorAnNeutral::process_event(PHCompositeNode *topNode)
{
  if (m_nevent % 1000 == 0)
  {
  ...
  if (Verbosity() >= 2)
  {
    std::cout << "Identification after emulator:" << std::endl;
    identify();
  }
...
int CaloTriggerEmulatorAnNeutral::ResetEvent(PHCompositeNode * /*topNode*/)
{
...
int CaloTriggerEmulatorAnNeutral::process_offline(PHCompositeNode *topNode)
{
  int sample_start = 1;
  ...
  std::vector<unsigned int> v_peak_sub_ped_zero;
  for (int i = sample_start; i < sample_end; i++)
  {
    v_peak_sub_ped_zero.push_back(0);
  }
  ...
  if (m_do_emcal)
  {
    ...
    if (!m_emcal_packets && !m_PacketNodesFlag)
    {
      // In newer DST, CaloPacketContainer CEMCPackets no longer present
      // Individual CaloPacket nodes (6001 -> 6128) are used instead
      for (int pid = m_packet_low_emcal; pid <= m_packet_high_emcal; pid++)
      {
        if (findNode::getClass<CaloPacket>(topNode, std::to_string(pid)))
        {
    ...
    for (int pid = m_packet_low_emcal; pid <= m_packet_high_emcal; pid++)
    {
      CaloPacket *packet = nullptr;
      if (m_PacketNodesFlag)
      {
        packet = findNode::getClass<CaloPacket>(topNode, std::to_string(pid));
      }
      else
      {
      ...
      // Global channel index to TowerInfoDefs key
      // Essential to retrieve user-friendly eta/phi indices
      unsigned int key = 0;

      if (!packet)
      {
        // if the packet is not available, fill with zeros.
        // Rare situation
        std::cout << "Missing packet " << pid << std::endl;
        for (int channel = 0; channel < 192; channel++)
        {
          key = TowerInfoDefs::encode_emcal(iwave);
          m_peak_sub_ped_emcal[key] = v_peak_sub_ped_zero;
          iwave++;
        }
        continue;
      }

      // Skipped IB, should be constant in all events
      unsigned int adc_skip_mask = 0;
      int nchannels = packet->iValue(0, "CHANNELS");

      // Empty packet
      // The recorded channel contains no channels
      // More common, mostly explains the difference in event number btw
      // DST_TRIGGERED_EVENT and DST_CALOFITTING
      // Also see CaloPacketSkimmer
      if (nchannels == 0)
      {
        for (int channel = 0; channel < 192; channel++)
        {
      ...
      for (int channel = 0; channel < nchannels; channel++)
      {
        ...
        bool ZS_cond = packet->iValue(channel, "SUPPRESSED");
        bool hot_cond = (cdbttree_hotMap != nullptr ? cdbttree_hotMap->GetIntValue(key, m_fieldname_hotMap) : false);
        if (ZS_cond || hot_cond)
        {
          // Skip ZS towers (and optionally bad towers from hot tower map)
          v_peak_sub_ped = v_peak_sub_ped_zero;
        }
        else
        {
          // Compute peak - pedestal from the Calo Packets
          for (int i = sample_start; i < sample_end; i++)
          {
      ...
      // Fill missing channels with zero
...
int CaloTriggerEmulatorAnNeutral::process_waveforms(PHCompositeNode *topNode)
{
  if (!m_isdata)
  {
    return process_sim();
  ...
  // If packets are stored in the PRDF node
  if (m_do_emcal)
  {
    if (Verbosity())
    {
...
int CaloTriggerEmulatorAnNeutral::process_sim()
{
  // Get range of waveforms
  if (Verbosity())
  {
  ...
  if (m_do_emcal)
  {
    ...
    // for each waveform, calculate the peak - pedestal given the sub-delay setting
...
int CaloTriggerEmulatorAnNeutral::process_primitives()
{
  int ip;
  int i;
  bool mask;
  ...
  if (m_do_emcal)
  {
    ...
    for (i = 0; i < m_n_primitives; i++, ip++)
    {
      ...
      for (int isum = 0; isum < m_n_sums; isum++)
      {
        ...
        // calculate sums for all samples, hence the vector.
...
int CaloTriggerEmulatorAnNeutral::process_organizer()
{
...
int CaloTriggerEmulatorAnNeutral::process_trigger()
{
...

  {
    ...
    for (TriggerPrimitiveContainerv1::Iter iter = range.first; iter != range.second; ++iter)
    {
      ...
      for (TriggerPrimitivev1::Iter iter_sum = sumrange.first; iter_sum != sumrange.second; ++iter_sum)
      {
        ...
        for (int is = 0; is < nsample; is++)
        {
          // Store all TriggerTile objects for which the tile ADC energy is above 2
          if (t_sum->at(is) > 1)
          {
            unsigned short sum_phi = static_cast<unsigned short>((TriggerDefs::getPrimitivePhiId_from_TriggerSumKey(sumk) * 2) + TriggerDefs::getSumPhiId(sumk));
            unsigned short sum_eta = static_cast<unsigned short>(TriggerDefs::getSumEtaId(sumk));
            m_tiles_emcal->AddTile(sum_eta, sum_phi, t_sum->at(is));
          }
  ...
  // Sort the tiles in decreasing order of energy
  m_tiles_emcal->SortTiles();

  return Fun4AllReturnCodes::EVENT_OK;
}

void CaloTriggerEmulatorAnNeutral::GetNodes(PHCompositeNode *topNode)
{
  if (Verbosity() >= 2)
  {
  ...
  if (m_do_emcal)
  {
    ...
    // Minimal info for tiles:
    // ADC energy, eta index (0->7), phi index (0->11)
    m_tiles_emcal = findNode::getClass<TriggerTile>(topNode, "TILES_EMCAL");
    if (!m_tiles_emcal)
    {
      std::cout << "No EMCAL tiles found... " << std::endl;
      exit(1);
    }
...
void CaloTriggerEmulatorAnNeutral::CreateNodes(PHCompositeNode *topNode)
{
  ...
  if (m_do_emcal)
  {
    std::string ll1_nodename = "TILES_EMCAL";
    TriggerTile *ll1out_tile = findNode::getClass<TriggerTile>(ll1Node, ll1_nodename);
    if (!ll1out_tile)
    {
      std::cout << "Create TILES_EMCAL" << std::endl;
      ll1out_tile = new TriggerTile();
      std::cout << "ll1out_tile = " << ll1out_tile << std::endl;
      PHIODataNode<PHObject> *LL1OutNode = new PHIODataNode<PHObject>(ll1out_tile, ll1_nodename, "PHObject");
      ll1Node->addNode(LL1OutNode);
    }

    ll1_nodename = "TRIGGERPRIMITIVES_EMCAL";
...
int CaloTriggerEmulatorAnNeutral::End(PHCompositeNode * /*topNode*/)
{
...
void CaloTriggerEmulatorAnNeutral::identify()
{
  std::cout << " CaloTriggerEmulatorAnNeutral: " << m_trigger << std::endl;
...
void CaloTriggerEmulatorAnNeutral::useHCALIN(bool use)
{
  m_do_hcalin = use;
  m_force_hcalin = true;
}
void CaloTriggerEmulatorAnNeutral::useHCALOUT(bool use)
{
  m_do_hcalout = use;
  m_force_hcalout = true;
}

void CaloTriggerEmulatorAnNeutral::useEMCAL(bool use)
{
  m_do_emcal = use;
  m_force_emcal = true;
}

unsigned int CaloTriggerEmulatorAnNeutral::getBits(unsigned int sum, TriggerDefs::TriggerId tid)
{
