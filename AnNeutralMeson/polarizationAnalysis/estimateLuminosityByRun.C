#include <uspin/SpinDBContent.h>
#include <uspin/SpinDBOutput.h>

R__LOAD_LIBRARY(libuspin.so)

void estimateLuminosityRun(int iB, int iScal, std::stringstream& GL1Pscalers, unsigned long& sumScalers, unsigned long runnumber, 
                           unsigned long& LuminosityUp, unsigned long& LuminosityDown,
                           double& pol, double& polerr, double& crossangle, double& crossangle_std);

void estimateLuminosityByRun(const std::string& runListName = "runs.txt", const std::string& outputfilename = "luminosity.root")
{
  std::ifstream fin;
  fin.open(runListName);
  if (!(fin.is_open()))
  {
    std::cerr << "Error: File " << runListName << " could not be opened.\n";
  }
  std::string line;

  // Polarizations and luminosities are defined differently for each beam and scaler
  const int nBeams = 2;
  const std::string beams[2] = {"blue", "yellow"};
  const int nScalers = 2;
  const std::string scalers[2] = {"mbdns", "zdcns"};

  // Book histograms
  TFile *outfile = new TFile(outputfilename.c_str(), "RECREATE");
  outfile->cd();

  // Polarization graphs
  TH1F *h_polarization[nBeams][nScalers];
  TH1F *h_relative_luminosity[nBeams][nScalers];
  TGraph *graph_polarization_run[nBeams][nScalers];
  TGraph *graph_relative_luminosity_run[nBeams][nScalers];
  TGraph *graph_luminosity_up_run[nBeams][nScalers];
  TGraph *graph_luminosity_down_run[nBeams][nScalers];
  TGraph *graph_crossangle_run[nBeams][nScalers];
  int runnumber;
  double Rmbd;
  double Rzdc;
  TTree *tree_relative_luminosity[nBeams];
  for (int iB = 0; iB < nBeams; iB++)
  {
    std::stringstream treename; treename << "tree_relative_luminosity_" << beams[iB]; 
    tree_relative_luminosity[iB] = new TTree(treename.str().c_str(), "relative luminosity tree");
    tree_relative_luminosity[iB]->Branch("runnumber", &runnumber, "runnumber/I"); 
    tree_relative_luminosity[iB]->Branch("mbd", &Rmbd, "mbd/D");
    tree_relative_luminosity[iB]->Branch("zdc", &Rzdc, "zdc/D");
    for (int iScal = 0; iScal < nScalers; iScal++) 
    {
      std::stringstream hname; hname << "h_polarization_" << beams[iB] << "_" << scalers[iScal];
      std::stringstream htitle; htitle << ";" << beams[iB] << " (" << scalers[iScal] << ") " << " beam polarization [%]; luminosity weighted distribution";
      h_polarization[iB][iScal] = new TH1F(hname.str().c_str(), htitle.str().c_str(), 50, 30, 65);
      hname.str(""); hname << "h_relative_luminosity_" << beams[iB] << "_" << scalers[iScal];
      htitle.str(""); htitle << ";" << beams[iB] << " (" << scalers[iScal] << ") " << " beam relative luminosity; luminosity weighted distribution";
      h_relative_luminosity[iB][iScal] = new TH1F(hname.str().c_str(), htitle.str().c_str(), 50, 0.9, 1.1);
      hname.str(""); hname << "graph_polarization_run_" << beams[iB] << "_" << scalers[iScal];
      htitle.str(""); htitle << ";run number;" << beams[iB] << " (" << scalers[iScal] << ") " << " beam polarization [%]";
      graph_polarization_run[iB][iScal] = new TGraph();
      graph_polarization_run[iB][iScal]->SetName(hname.str().c_str());
      graph_polarization_run[iB][iScal]->SetTitle(htitle.str().c_str());
      hname.str(""); hname << "graph_relative_luminosity_run_" << beams[iB] << "_" << scalers[iScal];
      htitle.str(""); htitle << ";run number;" << beams[iB] << " (" << scalers[iScal] << ") " << " beam relative luminosity";
      graph_relative_luminosity_run[iB][iScal] = new TGraph();
      graph_relative_luminosity_run[iB][iScal]->SetName(hname.str().c_str());
      graph_relative_luminosity_run[iB][iScal]->SetTitle(htitle.str().c_str());
      hname.str(""); hname << "graph_luminosity_up_run_" << beams[iB] << "_" << scalers[iScal];
      htitle.str(""); htitle << ";run number;" << beams[iB] << " (" << scalers[iScal] << ") " << " beam luminosity up";
      graph_luminosity_up_run[iB][iScal] = new TGraph();
      graph_luminosity_up_run[iB][iScal]->SetName(hname.str().c_str());
      graph_luminosity_up_run[iB][iScal]->SetTitle(htitle.str().c_str());
      hname.str(""); hname << "graph_luminosity_down_run_" << beams[iB] << "_" << scalers[iScal];
      htitle.str(""); htitle << ";run number;" << beams[iB] << " (" << scalers[iScal] << ") " << " beam luminosity down";
      graph_luminosity_down_run[iB][iScal] = new TGraph();
      graph_luminosity_down_run[iB][iScal]->SetName(hname.str().c_str());
      graph_luminosity_down_run[iB][iScal]->SetTitle(htitle.str().c_str());

      hname.str(""); hname << "graph_crossangle_run_" << beams[iB] << "_" << scalers[iScal];
      htitle.str(""); htitle << ";run number;" << beams[iB] << " (" << scalers[iScal] << ") " << " beam luminosity down";
      graph_crossangle_run[iB][iScal] = new TGraph();
      graph_crossangle_run[iB][iScal]->SetName(hname.str().c_str());
      graph_crossangle_run[iB][iScal]->SetTitle(htitle.str().c_str());
    }
  }
  
  unsigned long normalization[nBeams][nScalers] = {0};
  long double average_polarization[nBeams][nScalers] = {0};
  long double average_relative_luminosity[nBeams][nScalers] = {0};
  
  //std::cout << "runnumber beam scaler LuminosityUp LuminosityDown RelativeLuminosity" << std::endl;
  int count = -1;
  while (std::getline(fin, line))
  {
    runnumber = std::stoi(line);
    std::cout << "run = " << runnumber << std::endl;
    count++;
    for (int iB = 0; iB < nBeams; iB++)
    {
      for (int iScal = 0; iScal < nScalers; iScal++)
      {
        unsigned long LuminosityUp = 0;
        unsigned long LuminosityDown = 0;
        double pol = 0;
        double polerr = 0;
        double crossangle;
        double crossangle_std;
        std::stringstream GL1Pscalers;
        unsigned long sumScalers = 0;
        estimateLuminosityRun(iB, iScal, GL1Pscalers, sumScalers, runnumber, LuminosityUp, LuminosityDown, pol, polerr, crossangle, crossangle_std);

        unsigned long Luminosity = LuminosityUp + LuminosityDown;
        /*double blueRelativeLuminosity = (bluepol * (double) blueUpLuminosity + (1 - bluepol) * 0.5 * ((double) blueUpLuminosity + (double) blueDownLuminosity)) /
          (bluepol * (double) blueDownLuminosity + (1 - bluepol) * 0.5 * ((double) blueUpLuminosity + (double) blueDownLuminosity));
        */
        long double RelativeLuminosity = (long double) LuminosityUp / (long double) LuminosityDown;
        if (iScal == 0) Rmbd = RelativeLuminosity;
        else if (iScal == 1) Rzdc = RelativeLuminosity;
        
        if (false) {
          average_polarization[iB][iScal] += Luminosity * pol;
          average_relative_luminosity[iB][iScal] += (long double) Luminosity * (long double) LuminosityDown / (long double) LuminosityUp;
        } else {
          average_polarization[iB][iScal] += Luminosity * pol;
          average_relative_luminosity[iB][iScal] += (long double) Luminosity * (long double) LuminosityUp / (long double) LuminosityDown;
        }

        //average_relative_luminosity_blue += (float) blueLuminosity * ((float) blueUpLuminosity / (float) blueDownLuminosity);
        normalization[iB][iScal] += Luminosity;

        //std::cout << runnumber << " " << beams[iB] << " " << scalers[iScal] << " " << Luminosity << std::endl;
        //std::cout << runnumber << " " << beams[iB] << " " << scalers[iScal] << " " << LuminosityUp << " " << LuminosityDown << " " << RelativeLuminosity << std::endl;
        
        // fill histograms
        h_polarization[iB][iScal]->Fill(pol * 100, Luminosity);
        graph_polarization_run[iB][iScal]->SetPoint(count, runnumber, pol);
        h_relative_luminosity[iB][iScal]->Fill(RelativeLuminosity, Luminosity);
        graph_relative_luminosity_run[iB][iScal]->SetPoint(count, runnumber, RelativeLuminosity);
        graph_luminosity_up_run[iB][iScal]->SetPoint(count, runnumber, LuminosityUp);
        graph_luminosity_down_run[iB][iScal]->SetPoint(count, runnumber, LuminosityDown);
        graph_crossangle_run[iB][iScal]->SetPoint(count, runnumber, crossangle);
        //graph_crossangle_run[iB][iScal]->SetPointError(count, 0, crossangle_std);
      }
      //std::cout << runnumber << " " << beams
      tree_relative_luminosity[iB]->Fill();
    }
  }
  for (int iB = 0; iB < nBeams; iB++)
  {
    for (int iScal = 0; iScal < nScalers; iScal++)
    {
      average_polarization[iB][iScal] /= (long double) normalization[iB][iScal];
      average_relative_luminosity[iB][iScal] /= (long double) normalization[iB][iScal];
      
      std::cout << std::fixed << std::setprecision(10)
                << "Average values " << beams[iB] << " " << scalers[iScal] << ":\n"
                << "Pol = " << average_polarization[iB][iScal] << "  "
                << "RL = " << average_relative_luminosity[iB][iScal] << "\n";
      
      graph_polarization_run[iB][iScal]->Write();
      graph_relative_luminosity_run[iB][iScal]->Write();
      graph_luminosity_up_run[iB][iScal]->Write();
      graph_luminosity_down_run[iB][iScal]->Write();
      graph_crossangle_run[iB][iScal]->Write();
      fin.close();
    }
  }
  
  outfile->Write();
  outfile->Close();
  delete outfile;

  gSystem->Exit(0);

}

void estimateLuminosityRun(int iB, int iScal, std::stringstream& GL1Pscalers, unsigned long& sumScalers, unsigned long runnumber, 
                           unsigned long& LuminosityUp, unsigned long& LuminosityDown,
                           double& pol, double& polerr, double& crossangle, double& crossangle_std)
{
  SpinDBOutput spin_out("phnxrc");
  SpinDBContent *spin_cont = new SpinDBContentv1();
  spin_out.StoreDBContent(runnumber, runnumber);
  spin_out.GetDBContentStore(spin_cont, runnumber);

  // General information
  //std::cout << "Run number: " << runnumber << std::endl;

  // Spin patterns
  int spinpat[120] = {0};
  if (iB == 0)
  {
    for (int i = 0; i < 120; ++i)
    {
      spinpat[i] = -1 * spin_cont->GetSpinPatternBlue(i);
    }
  }
  else if (iB == 1)
  {
    for (int i = 0; i < 120; ++i)
    {
      spinpat[i] = -1 * spin_cont->GetSpinPatternYellow(i);
    }
  }

  crossangle = spin_cont->GetCrossAngle();
  crossangle_std = spin_cont->GetCrossAngleStd();

  // Beam polarization
  //double bluepol, yellpol, bluepolerr, yellpolerr;
  if (iB == 0) spin_cont->GetPolarizationBlue(0, pol, polerr);
  else if (iB == 1) spin_cont->GetPolarizationYellow(0, pol, polerr);
  pol /= 100; polerr /= 100;

  // GL1p Scalers
  // I don't know which one is the most relevant
  unsigned long scaler[120] = {0};
  //GL1Pscalers << "\"";
  sumScalers = 0;
  if (iScal == 0)
  {
    for (int i = 0; i < 111; ++i)
    {
      scaler[i] = spin_cont->GetScalerMbdNoCut(i); sumScalers += scaler[i];
    }
  } else if (iScal == 1)
  {
    for (int i = 0; i < 111; ++i)
    {
      scaler[i] = spin_cont->GetScalerZdcNoCut(i); sumScalers += scaler[i];
    }
  } else if (iScal == 2)
  {
    std::cout << "This should never happen!\n";
    for (int i = 0; i < 111; ++i)
    {
      scaler[i] = spin_cont->GetScalerMbdVertexCut(i); sumScalers += scaler[i];
    }
  }

  // Estimate luminosity:
  LuminosityUp = 0;
  LuminosityDown = 0;
  for (int i = 0; i < 120; i++)
  {
    if (spinpat[i] == 1)
      LuminosityUp += scaler[i];
    else if (spinpat[i] == -1)
      LuminosityDown += scaler[i];
  }
}

