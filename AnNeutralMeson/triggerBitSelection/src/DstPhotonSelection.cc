...
DstPhotonSelection::DstPhotonSelection(const std::string &name, const int runnb)
  : SubsysReco(name)
  , runnumber(runnb)
{
}

DstPhotonSelection::~DstPhotonSelection()
{
}

int DstPhotonSelection::Init(PHCompositeNode *)
{  
  return 0;
}

int DstPhotonSelection::InitRun(PHCompositeNode *topNode)
{
  ...
  // Determine the scaledown number of each trigger bit
...
int DstPhotonSelection::process_event(PHCompositeNode *)
{
  ...
  if (!(trigger_mbd_photon_3 || trigger_mbd_photon_4 || trigger_mbd_photon_5))
    return Fun4AllReturnCodes::ABORTEVENT;
  
  return Fun4AllReturnCodes::EVENT_OK;
}

int DstPhotonSelection::End(PHCompositeNode *)
{
  return 0;
}
