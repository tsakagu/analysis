...
void fit_argusmodified_2_eta(const std::string &production_date = "01312026",
                             const std::string &selection_label = "MBD_pt_l3_0mrad",
                             ...
                             const std::string &plots_folder_template = "")
{
  ...
  const float etaBounds[nEtaBins+1] = {-2.00, -1.05, -0.86, -0.61, 0.0, 0.61, 0.86, 1.05, 2.0};

  // Read mean eta values
  std::string input_csv_name = folder_inputs + "/input_eta_mean_" + production_date + "_" + selection_label + ".csv";
  ...
  //TFile *outputfile = new TFile("eta_ratios_ana450_p009.root", "RECREATE");
  TGraphErrors *eta_ratios_pi0 = new TGraphErrors();
  eta_ratios_pi0->SetName("eta_background_ratios_pi0");
  eta_ratios_pi0->SetTitle("#pi^{0} background ratio");
  eta_ratios_pi0->GetXaxis()->SetTitle("#eta");
  eta_ratios_pi0->GetYaxis()->SetTitle("background fraction [%]");
  TGraphErrors *eta_ratios_eta = new TGraphErrors();
  eta_ratios_eta->SetTitle("#eta background ratio");
  eta_ratios_eta->GetXaxis()->SetTitle("#eta");
  eta_ratios_eta->GetYaxis()->SetTitle("background ratio [%]");
  eta_ratios_eta->SetName("eta_background_ratios_eta");

  std::string plots_folder = plots_folder_template + "/plots_mass_argusmodified_2_eta/analysis_ana509/ana509_" + production_date + "_sigma_3/" + selection_label + "/";

  gSystem->Exec(("mkdir -p " + plots_folder).c_str());
  double statSum = 0; // Count nentries (for debug if suspicious uncertainties)

  for (int ieta = 0; ieta < nEtaBins/2; ieta++)
  {
    std::string filename = inputfolder_analysis + "/analysis_complete_ana509_" + production_date + "_" + selection_label + "_symmetric.root";
    
    std::string hname = "h_pair_mass_eta_" + std::to_string(ieta) + "_sym";
    SetsPhenixStyle();
    ...
    statList << "bin : #eta in [" << std::fixed << std::setprecision(2) << etaBounds[ieta] << ", " << etaBounds[ieta+1] << "]: nentries = " << h_pair_mass->GetEntries() << std::endl;
    statList.close();
    ...
    if (ieta == 0) fitMethodBkg1->SetFitRange({{0.05, 0.11}, {0.20, 0.32}});
    ...
    if (pi0_ratio > 0 && pi0_ratio < 1) {
      eta_ratios_pi0->SetPoint(ieta, etaMeans[0][ieta], pi0_ratio * 100);
      eta_ratios_pi0->SetPointError(ieta, 0, pi0_ratio_err * 100);
      eta_ratios_pi0->SetPoint(nEtaBins-1-ieta, etaMeans[0][nEtaBins-1-ieta], pi0_ratio * 100);
      eta_ratios_pi0->SetPointError(nEtaBins-1-ieta, 0, pi0_ratio_err * 100);
    } else {
      eta_ratios_pi0->SetPoint(ieta, etaMeans[0][ieta], 100);
      eta_ratios_pi0->SetPointError(ieta, 0, 0);
      eta_ratios_pi0->SetPoint(nEtaBins-1-ieta, etaMeans[0][nEtaBins-1-ieta], 100);
      eta_ratios_pi0->SetPointError(nEtaBins-1-ieta, 0, 0);
    }
    if (eta_ratio > 0 && eta_ratio < 1) {
      eta_ratios_eta->SetPoint(ieta, etaMeans[0][ieta], eta_ratio * 100);
      eta_ratios_eta->SetPointError(ieta, 0, eta_ratio_err * 100);
      eta_ratios_eta->SetPoint(nEtaBins-1-ieta, etaMeans[0][nEtaBins-1-ieta], eta_ratio * 100);
      eta_ratios_eta->SetPointError(nEtaBins-1-ieta, 0, eta_ratio_err * 100);
    } else {
      eta_ratios_eta->SetPoint(ieta, etaMeans[0][ieta], 100);
      eta_ratios_eta->SetPointError(ieta, 0, 0);
      eta_ratios_eta->SetPoint(nEtaBins-1-ieta, etaMeans[0][nEtaBins-1-ieta], 100);
      eta_ratios_eta->SetPointError(nEtaBins-1-ieta, 0, 0);
    }

    std::ofstream ratioList;
    ratioList.open((plots_folder + "/ratioList.txt"), ios::out | ios::app);
    ratioList << "bin : #eta in [" << std::fixed << std::setprecision(2) << etaBounds[ieta] << ", " << etaBounds[ieta+1] << "]: r_{pi0} = " << pi0_ratio << ", r_{eta} = " << eta_ratio << std::endl;
    ratioList.close();
    ...
    */
    ...
    std::string outfilename_root = plots_folder + "/parametric_fit_mass_eta_" + std::to_string(ieta) + ".root";
    ...
    std::string canvas_name = "canvas_pi0_eta_mass_eta_" + std::to_string(ieta);
    ...
    // Draw fits
    // fit_bkg_1->SetRange(offset, borderFitMin);
    ...
    // fit_bkg_2->SetRange(borderFitMax,1.0);
    ...
    if (ieta < nEtaBins) {
      std::stringstream bin_label;
      bin_label << std::fixed << std::setprecision(2) << (-etaBounds[ieta+1]) << " < #eta < " << (-etaBounds[ieta]);
      latex.DrawLatex(0.42,0.87, bin_label.str().c_str());
    }
  ...
  std::string canvas_name = "canvas_eta_background_ratios";
  TCanvas *canvas = new TCanvas(canvas_name.c_str(), "c", 1600, 900);
  TMultiGraph *mg = new TMultiGraph();
  mg->GetXaxis()->SetTitle("#eta");
  mg->GetYaxis()->SetTitle("Background Fraction [%]");
  eta_ratios_pi0->SetMarkerSize(2.5);
  eta_ratios_pi0->SetMarkerStyle(kFullCircle);
  eta_ratios_pi0->SetMarkerColor(kBlue);
  eta_ratios_eta->SetMarkerSize(2.5);
  eta_ratios_eta->SetMarkerStyle(kFullSquare);
  eta_ratios_eta->SetMarkerColor(kRed);
  mg->Add(eta_ratios_pi0);
  mg->Add(eta_ratios_eta);
  mg->SetMinimum(0);
  mg->SetMaximum(100);
  ...
  /*TPad *p = new TPad("p","p",0.,0.,1.,1.); p->SetFillStyle(0); p->Draw(); p->cd();
  TBox *whiteBox = new TBox(0.245, 0.59, 0.445, 0.78);
  whiteBox->SetFillColor(kBlack);
  whiteBox->SetLineColor(kBlack);
  whiteBox->Draw();
  canvas->cd();*/
  TLegend *legend = new TLegend(0.2, 0.5, 0.45, 0.7);
  //TLegend *legend = new TLegend(0.6, 0.2, 0.9, 0.4);
  legend->AddEntry(eta_ratios_pi0, "p^{#uparrow}p #rightarrow #pi^{0} X");
  legend->AddEntry(eta_ratios_eta, "p^{#uparrow}p #rightarrow #eta X");
  ...
  canvas_name = "canvas_eta_background_ratios_pi0";
  canvas = new TCanvas("canvas_eta_background_ratios_pi0", "c", 1600, 900);
  canvas->cd();
  eta_ratios_pi0->SetMinimum(0);
  eta_ratios_pi0->SetMaximum(100);
  //eta_ratios_pi0->SetLineWidth(0);
  eta_ratios_pi0->SetMarkerSize(1.5);
  eta_ratios_pi0->SetMarkerStyle(kFullCircle);
  eta_ratios_pi0->SetMarkerColor(kBlue);
  eta_ratios_pi0->Draw("AP");
  ...
  canvas_name = "canvas_eta_background_ratios_eta";
  canvas = new TCanvas("canvas_eta_background_ratios_eta", "c", 1600, 900);
  canvas->cd();
  eta_ratios_eta->SetMinimum(70);
  eta_ratios_eta->SetMaximum(100);
  //eta_ratios_eta->SetLineWidth(0);
  eta_ratios_eta->SetMarkerSize(1.5);
  eta_ratios_eta->SetMarkerStyle(kFullCircle);
  eta_ratios_eta->SetMarkerColor(kRed);
  eta_ratios_eta->Draw();

  std::ofstream ratio_csv;
  ratio_csv.open((outputfolder_ratios + "/eta_ratios_" + production_date + "_" + selection_label + ".csv"));
  for (int iEta = 0; iEta < nEtaBins - 1; iEta++) ratio_csv << eta_ratios_pi0->GetPointY(iEta) / 100. << ", ";
  ratio_csv << eta_ratios_pi0->GetPointY(nEtaBins - 1) / 100. << "\n";
  for (int iEta = 0; iEta < nEtaBins - 1; iEta++) ratio_csv << eta_ratios_eta->GetPointY(iEta) / 100. << ", ";
  ratio_csv << eta_ratios_eta->GetPointY(nEtaBins - 1) / 100. << "\n";
  for (int iEta = 0; iEta < nEtaBins - 1; iEta++) ratio_csv << 0  << ", ";
  ratio_csv << 0 << "\n";
  for (int iEta = 0; iEta < nEtaBins - 1; iEta++) ratio_csv << 0 << ", ";
  ratio_csv << 0 << "\n";
  ratio_csv.close();
