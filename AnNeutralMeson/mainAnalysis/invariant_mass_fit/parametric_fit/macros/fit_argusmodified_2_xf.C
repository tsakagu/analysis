...
void fit_argusmodified_2_xf(const std::string &production_date = "01312026",
...
{
  // Warning: for eta meson, you can actually go beyond xF = 0.15 (to fix later) 
  const int nXfBins = 8;
  const float xfBounds[nXfBins+1] = {-0.200, -0.048, -0.035, -0.022, 0.0, 0.022, 0.035, 0.048, 0.200};

  // Read mean xf values
  std::string input_csv_name = folder_inputs + "/input_xf_mean_" + production_date + "_" + selection_label + ".csv";
  ...
  //TFile *outputfile = new TFile("xf_ratios_ana450_p009.root", "RECREATE");
  TGraphErrors *xf_ratios_pi0 = new TGraphErrors();
  xf_ratios_pi0->SetName("xf_background_ratios_pi0");
  xf_ratios_pi0->SetTitle("#pi^{0} background ratio");
  xf_ratios_pi0->GetXaxis()->SetTitle("x_F");
  xf_ratios_pi0->GetYaxis()->SetTitle("background fraction [%]");
  TGraphErrors *xf_ratios_eta = new TGraphErrors();
  xf_ratios_eta->SetTitle("#eta background ratio");
  xf_ratios_eta->GetXaxis()->SetTitle("x_F");
  xf_ratios_eta->GetYaxis()->SetTitle("background ratio [%]");
  xf_ratios_eta->SetName("xf_background_ratios_eta");

  std::string plots_folder = plots_folder_template + "/plots_mass_argusmodified_2_xf/analysis_ana509/ana509_" + production_date + "_sigma_3/" + selection_label + "/";
  ...
  for (int ixf = 0; ixf < nXfBins/2; ixf++)
  {
    ...
    std::string hname = "h_pair_mass_xf_" + std::to_string(ixf) + "_sym";
    SetsPhenixStyle();
    ...
    statList << "bin : x_F in [" << std::fixed << std::setprecision(3) << xfBounds[ixf] << ", " << xfBounds[ixf+1] << "]: nentries = " << h_pair_mass->GetEntries() << std::endl;
    statList.close();
    ...
    if (ixf == 0) fitMethodBkg1->SetFitRange({{0.05, 0.11}, {0.20, 0.32}});
    ...
    // // show
    // TCanvas *canvas_debug = new TCanvas("canvas_debug");
    // canvas_debug->cd();
    // h_pair_mass->Draw();
    // fit_eta->SetLineColor(kRed);
    // fit_eta->Draw("same");
    // canvas_debug->Modified();
    // canvas_debug->Update();
    // canvas_debug->Draw();
    // sleep(10);
    ...
    if (pi0_ratio > 0 && pi0_ratio < 1) {
      xf_ratios_pi0->SetPoint(ixf, xfMeans[0][ixf], pi0_ratio * 100);
      xf_ratios_pi0->SetPointError(ixf, 0, pi0_ratio_err * 100);
      xf_ratios_pi0->SetPoint(nXfBins-1-ixf, xfMeans[0][nXfBins-1-ixf], pi0_ratio * 100);
      xf_ratios_pi0->SetPointError(nXfBins-1-ixf, 0, pi0_ratio_err * 100);
    } else {
      xf_ratios_pi0->SetPoint(ixf, xfMeans[0][ixf], 100);
      xf_ratios_pi0->SetPointError(ixf, 0, 0);
      xf_ratios_pi0->SetPoint(nXfBins-1-ixf, xfMeans[0][nXfBins-1-ixf], 100);
      xf_ratios_pi0->SetPointError(nXfBins-1-ixf, 0, 0);
    }
    if (eta_ratio > 0 && eta_ratio < 1) {
      xf_ratios_eta->SetPoint(ixf, xfMeans[0][ixf], eta_ratio * 100);
      xf_ratios_eta->SetPointError(ixf, 0, eta_ratio_err * 100);
      xf_ratios_eta->SetPoint(nXfBins-1-ixf, xfMeans[0][nXfBins-1-ixf], eta_ratio * 100);
      xf_ratios_eta->SetPointError(nXfBins-1-ixf, 0, eta_ratio_err * 100);
    } else {
      xf_ratios_eta->SetPoint(ixf, xfMeans[0][ixf], 100);
      xf_ratios_eta->SetPointError(ixf, 0, 0);
      xf_ratios_eta->SetPoint(nXfBins-1-ixf, xfMeans[0][nXfBins-1-ixf], 100);
      xf_ratios_eta->SetPointError(nXfBins-1-ixf, 0, 0);
    }

    std::ofstream ratioList;
    ratioList.open((plots_folder + "/ratioList.txt"), ios::out | ios::app);
    ratioList << "bin : x_F in [" << std::fixed << std::setprecision(3) << xfBounds[ixf] << ", " << xfBounds[ixf+1] << "]: r_{pi0} = " << pi0_ratio << ", r_{eta} = " << eta_ratio << std::endl;
    ratioList.close();
    ...
    */
    ...
    std::string outfilename_root = plots_folder + "/parametric_fit_mass_xf_" + std::to_string(ixf) + ".root";
    ...
    std::string canvas_name = "canvas_pi0_eta_mass_xf_" + std::to_string(ixf);
    ...
    if (ixf < nXfBins) {
      ...
      bin_label << std::fixed << std::setprecision(3) << (-xfBounds[ixf+1]) << " < x_{F} < " << (-xfBounds[ixf]);
  ...
  std::string canvas_name = "canvas_xf_background_ratios";
  TCanvas *canvas = new TCanvas(canvas_name.c_str(), "c", 1600, 900);
  TMultiGraph *mg = new TMultiGraph();
  mg->GetXaxis()->SetTitle("x_F");
  mg->GetYaxis()->SetTitle("Background Fraction [%]");
  xf_ratios_pi0->SetMarkerSize(2.5);
  xf_ratios_pi0->SetMarkerStyle(kFullCircle);
  xf_ratios_pi0->SetMarkerColor(kBlue);
  xf_ratios_eta->SetMarkerSize(2.5);
  xf_ratios_eta->SetMarkerStyle(kFullSquare);
  xf_ratios_eta->SetMarkerColor(kRed);
  mg->Add(xf_ratios_pi0);
  mg->Add(xf_ratios_eta);
  mg->SetMinimum(0);
  mg->SetMaximum(100);
  ...
  legend->AddEntry(xf_ratios_pi0, "p^{#uparrow}p #rightarrow #pi^{0} X");
  legend->AddEntry(xf_ratios_eta, "p^{#uparrow}p #rightarrow #eta X");
  ...
  canvas_name = "canvas_xf_background_ratios_pi0";
  canvas = new TCanvas("canvas_xf_background_ratios_pi0", "c", 1600, 900);
  canvas->cd();
  xf_ratios_pi0->SetMinimum(0);
  xf_ratios_pi0->SetMaximum(100);
  //xf_ratios_pi0->SetLineWidth(0);
  xf_ratios_pi0->SetMarkerSize(1.5);
  xf_ratios_pi0->SetMarkerStyle(kFullCircle);
  xf_ratios_pi0->SetMarkerColor(kBlue);
  xf_ratios_pi0->Draw("AP");
  ...
  canvas_name = "canvas_xf_background_ratios_eta";
  canvas = new TCanvas("canvas_xf_background_ratios_eta", "c", 1600, 900);
  canvas->cd();
  xf_ratios_eta->SetMinimum(70);
  xf_ratios_eta->SetMaximum(100);
  //xf_ratios_eta->SetLineWidth(0);
  xf_ratios_eta->SetMarkerSize(1.5);
  xf_ratios_eta->SetMarkerStyle(kFullCircle);
  xf_ratios_eta->SetMarkerColor(kRed);
  xf_ratios_eta->Draw();

  std::ofstream ratio_csv;
  ratio_csv.open((outputfolder_ratios + "/xf_ratios_" + production_date + "_" + selection_label + ".csv"));
  for (int iXf = 0; iXf < nXfBins - 1; iXf++) ratio_csv << xf_ratios_pi0->GetPointY(iXf) / 100. << ", ";
  ratio_csv << xf_ratios_pi0->GetPointY(nXfBins - 1) / 100. << "\n";
  for (int iXf = 0; iXf < nXfBins - 1; iXf++) ratio_csv << xf_ratios_eta->GetPointY(iXf) / 100. << ", ";
  ratio_csv << xf_ratios_eta->GetPointY(nXfBins - 1) / 100. << "\n";
  for (int iXf = 0; iXf < nXfBins - 1; iXf++) ratio_csv << 0  << ", ";
  ratio_csv << 0 << "\n";
  for (int iXf = 0; iXf < nXfBins - 1; iXf++) ratio_csv << 0 << ", ";
  ratio_csv << 0 << "\n";
  ratio_csv.close();
