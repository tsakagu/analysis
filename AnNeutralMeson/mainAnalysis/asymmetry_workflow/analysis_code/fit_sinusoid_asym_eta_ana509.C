...
void fit_sinusoid_asym_eta_ana509(const std::string &production_date = "01312026",
                                  ...
                                  bool gpr_ratio = false)
{
  ...
  std::string inputfilename = folder_asymmetry + "/analysis_asymmetry_eta_complete_ana509_" + production_date + "_" + selection_label + "_averaged.root";
  std::string outputfilename = folder_asymmetry + "/analysis_asymmetry_eta_complete_ana509_" + production_date + "_" + selection_label + "_averaged_fitted" + ratio_suffix + ".root";

  std::string plots_folder = "plots_asymmetry/ana509_p022/analysis_" + production_date + "/eta_asymmetries/" + selection_label + "/phi_asymmetries/runs";
  ...
  float etaMeans[2][nEtaBins] = {0};
  {
    std::string input_csv_name = folder_csv_inputs + "/input_eta_mean_" + production_date + "_" + selection_label + ".csv";
    std::string line;
  ...
  float bkg_ratio[2][2][nEtaBins] = {0};
  {
    std::string input_csv_name = folder_csv_ratios + "/eta_ratios_" + production_date + "_" + selection_label + ratio_suffix + ".csv";
    std::string line;
    std::stringstream linestream;
    std::string entry;
    std::ifstream input_csv;
    ...
    for (int ieta = 0; ieta < nEtaBins; ieta++) {
      ...
      bkg_ratio[1][0][ieta] = etaRatioErr;
    }
    std::getline(input_csv, line);
    ...
    for (int ieta = 0; ieta < nEtaBins; ieta++) {
      ...
      float etaRatioErr = std::stof(entry);
      bkg_ratio[1][1][ieta] = etaRatioErr;
    }
  }
  ...
  for (int iB = 0; iB < nBeams; iB++)
  {
    for (int iP = 0; iP < nParticles; iP++)
    {
      for (int imethod = 0; imethod < nMethods; imethod++)
      {
        ...
        for (int ieta = 0; ieta < nEtaBins; ieta++)
        {
          float mean_eta = etaMeans[iP][ieta];
          graph_AN_raw[iB][iP][imethod]->SetPoint(ieta, mean_eta, 0);
          graph_AN_raw_offset[iB][iP][imethod]->SetPoint(ieta, mean_eta, 0);
          graph_AN_bkg[iB][iP][imethod]->SetPoint(ieta, mean_eta, 0);
          graph_AN_bkg_offset[iB][iP][imethod]->SetPoint(ieta, mean_eta, 0);
          graph_AN_peak[iB][iP][imethod]->SetPoint(ieta, mean_eta, 0);
        }
      }
    }
  }
      ...
      for (int iP = 0; iP < nParticles; iP++)
      {
        for (int ieta = 0; ieta < nEtaBins; ieta++)
        {
          for (int imethod = 0; imethod < nMethods; imethod++)
          {
            ...
            graph_AN_title << "#eta in [" << std::fixed << std::setprecision(1) << etaBins[ieta][0] << ", " << etaBins[ieta][1] << "] (" << (imethod == 0 ? "Geometric" : "Rel. Lumi.")  << ", " << beams[iB] << " beam); #phi; " << (iR == 0 ? "Raw Asymmetry" : "Bkg Asymmetry");
            ...
            if (iR == 0)
            {
              graph_AN_raw[iB][iP][imethod]->SetPointY(ieta, epsilon);
              graph_AN_raw[iB][iP][imethod]->SetPointError(ieta, 0, depsilon);
              graph_AN_raw_offset[iB][iP][imethod]->SetPointY(ieta, offset);
              graph_AN_raw_offset[iB][iP][imethod]->SetPointError(ieta, 0, doffset);
            }
            else
            {
              graph_AN_bkg[iB][iP][imethod]->SetPointY(ieta, epsilon);
              graph_AN_bkg[iB][iP][imethod]->SetPointError(ieta, 0, depsilon);
              graph_AN_bkg_offset[iB][iP][imethod]->SetPointY(ieta, offset);
              graph_AN_bkg_offset[iB][iP][imethod]->SetPointError(ieta, 0, doffset);
            }
          }
        }
      }
  ...
  for (int iB = 0; iB < nBeams; iB++)
  {
    for (int iP = 0; iP < nParticles; iP++)
    {
      for (int ieta = 0; ieta < nEtaBins; ieta++)
      {
        for (int imethod = 0; imethod < nMethods; imethod++)
        {
          float R = bkg_ratio[0][iP][ieta];
          R = (R == 0 ? 1 : R);
          float AN_raw = graph_AN_raw[iB][iP][imethod]->GetPointY(ieta);
          float AN_raw_err = graph_AN_raw[iB][iP][imethod]->GetErrorY(ieta);
          float AN_bkg = graph_AN_bkg[iB][iP][imethod]->GetPointY(ieta);
          float AN_bkg_err = graph_AN_bkg[iB][iP][imethod]->GetErrorY(ieta);
          float R_err = bkg_ratio[1][iP][ieta];
          ...
          graph_AN_peak[iB][iP][imethod]->SetPointY(ieta, AN_peak_val);
          graph_AN_peak[iB][iP][imethod]->SetPointError(ieta, 0, AN_peak_err);
        }
      }
    }
  }
  ...
  for (int iB = 0; iB < nBeams; iB++)
  {
    for (int iP = 0; iP < nParticles; iP++)
    {
      ...
      {
        std::stringstream graph_AN_title;
        graph_AN_title << beams[iB] << " asymmetry; #eta; Raw asymmetry";
        ...
        graph_AN_title.str("");
        graph_AN_title << beams[iB] << " asymmetry; #eta; Raw constant offset";
        ...
        graph_AN_title.str("");
        graph_AN_title << beams[iB] << " asymmetry; #eta; Background asymmetry";
        ...
        graph_AN_title.str("");
        graph_AN_title << beams[iB] << " asymmetry; #eta; Background constant offset";
        ...
        graph_AN_title.str("");
        graph_AN_title << beams[iB] << " asymmetry; #eta; Corrected asymmetry";
