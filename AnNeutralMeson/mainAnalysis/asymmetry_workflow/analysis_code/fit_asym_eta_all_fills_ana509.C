...
TGraphErrors* SqrtGraph(TH1* phi_up, TH1* phi_down, TH1F* eta_yield_left_down, TH1F *eta_yield_left_up, TH1F* eta_yield_right_down, TH1F *eta_yield_right_up, int ieta, double Pol);
...
void fit_asym_eta_ana509(const std::string &production_date,
...
void fit_asym_eta_all_fills_ana509(const std::string &production_date = "01312026",
                                   ...
                                   const std::string &outputfolder_asymmetry = "")
{
  if (separate_fills)
  {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      fit_asym_eta_ana509(production_date, selection_label, inputfolder_mass, inputfolder_analysis, outputfolder_asymmetry, RL_blue, RL_yellow, P_blue, P_yellow, fillnumber);
    }
  f->Close();
  }
  else
  {
    // Average values
    fit_asym_eta_ana509(production_date, selection_label, inputfolder_mass, inputfolder_analysis, outputfolder_asymmetry, 1.0012080359, 1.0013263725, 0.5082929939, 0.5167388276);
  }
  gSystem->Exit(0);
}
                         ...
                         int inputfill)
{
  ...
  if (inputfill == 0)
  {
    inputfilename = inputfolder_analysis + "/analysis_eta_complete_ana509_" + production_date + "_" + selection_label + ".root";
  ...
  else
  {
    ...
    std::string outputfolder = outputfolder_asymmetry + "/analysis_fills_asymmetry_eta_complete_ana509_" + production_date + "_" + selection_label;
    gSystem->Exec(("mkdir -p " + outputfolder).c_str());
  ...
  TH1F *h_yield[nBeams][nParticles][nRegions][nEtaBins][nSpins];
        ...
        for (int iS = 0; iS < nSpins; iS++)
          for (int ieta = 0; ieta < nEtaBins; ieta++)
          {
            ...
            h_yield_name << "h_yield_" << beams[iB] << "_" << particle[iP] << "_" << regions[iR] << "_"
                         << "eta_" << ieta << "_"
                         << spins[iS];
            h_yield[iB][iP][iR][ieta][iS] = (TH1F*)inputfile->Get(h_yield_name.str().c_str());
          }

  TH1F *h_yield_eta[nBeams][nParticles][nRegions][nSpins][nSides];
  for (int iB = 0; iB < nBeams; iB++)
    for (int iP = 0; iP < nParticles; iP++)
      for (int iR = 0; iR < nRegions; iR++)
        for (int iS = 0; iS < nSpins; iS++)
          ...
          {
            std::stringstream h_yield_name;
            h_yield_name << "h_yield_eta_" << beams[iB] << "_" << particle[iP] << "_" << regions[iR] << "_"
            ...
            h_yield_title << "statistics distribution (" << sides[iSi] << " " << spins[iS] << " " << " " << beams[iB] << " " << regions[iR] << ")"
                          << "; #eta [rad]; Counts";
            h_yield_eta[iB][iP][iR][iS][iSi] = new TH1F(h_yield_name.str().c_str(),
                                                       h_yield_title.str().c_str(),
                                                       nEtaBins, 0, (float) nEtaBins);
          }

  std::string plots_folder = "plots_asymmetry/ana509_p022/analysis_" + production_date + "/eta_asymmetries/phi_asymmetries/" + selection_label + "/";
  ...
  for (int iB = 0; iB < nBeams; iB++)
  {
    ...
    for (int iP = 0; iP < nParticles; iP++)
    {
      for (int ieta = 0; ieta < nEtaBins; ieta++)
      {
        ...
        {
          // raw asymmetry
          TH1F *phi_up = h_yield[iB][iP][0][ieta][0];
          TH1F *phi_down = h_yield[iB][iP][0][ieta][1];
          TGraphErrors *graph_AN = (imethod == 0 ? SqrtGraph(phi_up, phi_down, h_yield_eta[iB][iP][0][0][0], h_yield_eta[iB][iP][0][1][0], h_yield_eta[iB][iP][0][0][1], h_yield_eta[iB][iP][0][1][1], ieta, P) : RelLumGraph(phi_up, phi_down, RL, P));

          std::stringstream graph_AN_name;
          graph_AN_name << "graph_asym_" << beams[iB] << "_" << particle[iP] << "_peak_eta_" << ieta << "_" << methods[imethod];
          ...
          graph_title << "#eta #in [" << std::fixed << std::setprecision(1) << etaBins[ieta][0] << ", " << etaBins[ieta][1] << "] rad; #phi; raw asymmetry";
          graph_AN->SetTitle(graph_title.str().c_str());
          graph_AN->Write();
          delete graph_AN; // deleting the file should be enough
          
          // bkg asymmetry
          phi_up = h_yield[iB][iP][1][ieta][0];
          phi_down = h_yield[iB][iP][1][ieta][1];
          graph_AN = (imethod == 0 ? SqrtGraph(phi_up, phi_down, h_yield_eta[iB][iP][1][0][0], h_yield_eta[iB][iP][1][1][0], h_yield_eta[iB][iP][1][0][1], h_yield_eta[iB][iP][1][1][1], ieta, P) : RelLumGraph(phi_up, phi_down, RL, P));
          if (graph_AN->GetErrorY(1) == graph_AN->GetErrorY(1) && graph_AN->GetErrorY(1) < 0)
          {
            std::cout << "weird error: " << graph_AN->GetErrorY(1) << " at indices (" << iB << "," << iP << "," << 1 << "," << ieta << "," << imethod << ")" << std::endl;
          }
          
          graph_AN_name.str("");
          graph_AN_name << "graph_asym_" << beams[iB] << "_" << particle[iP] << "_side_eta_" << ieta << "_" << methods[imethod];
          graph_AN->SetName(graph_AN_name.str().c_str());
          
          canvas_name = "canvas_" + graph_AN_name.str().substr(graph_AN_name.str().find("graph_asym_") + 11);
          graph_title.str("");
          graph_title << "#eta #in [" << std::fixed << std::setprecision(1) << etaBins[ieta][0] << ", " << etaBins[ieta][1] << "] rad; #phi; bkg asymmetry";
          graph_AN->SetTitle(graph_title.str().c_str());
          graph_AN->Write();
          delete graph_AN;
        }
  ...
  outputfile->cd();
  // Store the eta histograms (debug essentially)
  if (inputfill == 0)
  {
    for (int iB = 0; iB < nBeams; iB++)
    {
        ...
        for (int iR = 0; iR < nRegions; iR++)
        {
          for (int imethod = 0; imethod < nMethods; imethod++)
          {
            for (int iS = 0; iS < nSpins; iS++)
            {
              for (int iSi = 0; iSi < nSides; iSi++)
              {
                h_yield_eta[iB][iP][iR][iS][iSi]->Write();
              }
            }
          }
        }
...
TGraphErrors* SqrtGraph(TH1* phi_up, TH1* phi_down, TH1F* eta_yield_left_down, TH1F *eta_yield_left_up, TH1F* eta_yield_right_down, TH1F *eta_yield_right_up, int ieta, double Pol)
{
  ...
  for (int i = 0; i < nbins; i++)
  {
    ...
    if (i >= 3 && i <= 8)
    {
      eta_yield_left_down->Fill(ieta, NLdown);
      eta_yield_left_up->Fill(ieta, NLup);
      eta_yield_right_down->Fill(ieta, NRdown);
      eta_yield_right_up->Fill(ieta, NRup);
    }
