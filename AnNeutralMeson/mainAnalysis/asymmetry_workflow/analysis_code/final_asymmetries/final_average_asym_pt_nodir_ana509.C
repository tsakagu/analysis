...
void final_average_asym_pt_nodir_ana509(bool gpr_ratio = false,
                                        const std::string& special_tag = "")
{
  ...
  TFile *outputfile = new TFile(("full_average_pt_nodir" + special_tag + ratio_suffix + ".root").c_str(), "RECREATE"); 
  ...
  for (int iTrigger = 0; iTrigger < nTriggers; iTrigger++) {
    for (int iCrossingAngle = 0; iCrossingAngle < nCrossingAngles; iCrossingAngle++) {
      for (int iParticle = 0; iParticle < nParticles; iParticle++) {
        std::string inputfilename = inputfolder + "/analysis_asymmetry_nodir_complete_ana509_01312026" + special_tag + "_" + trigger_labels[iTrigger] + pt_thresholds[iParticle] + "_" + crossing_angles[iCrossingAngle] + "_averaged_fitted" + ratio_suffix + ".root";
  ...
  for (int iP = 0; iP < nParticles; iP++) {
    std::stringstream graphname;
    graphname << "graph_AN_pt_peak_" << particles[iP];
    ...
    shift_graph(graph_AN_final[iP][1], 0.1);
    ...
    double y_bound = 0;
    if (iP == 0) y_bound = GetAbsMaxInRange(graph_AN_final[iP][1], 1.0, 10.0);
    else if (iP == 1) y_bound = GetAbsMaxInRange(graph_AN_final[iP][1], 2.0, 20.0);
  ...
  for (int iP = 0; iP < nParticles; iP++) {
    std::stringstream graphname;
    graphname << "graph_AN_pt_peak_beams_" << particles[iP];
    std::stringstream canvas_name;
    canvas_name << "canvas_" <<  graphname.str();
    TCanvas *canvas = new TCanvas(canvas_name.str().c_str(), "c");// 1600, 900);
    canvas->cd();
    ...
    shift_graph(graph_AN_triggerA_angleA[1][iP][0], 0.1);
  ...
  for (int iP = 0; iP < nParticles; iP++) {
    std::stringstream graphname;
    graphname << "graph_AN_pt_peak_angles_" << particles[iP];
    std::stringstream canvas_name;
    canvas_name << "canvas_" <<  graphname.str();
    TCanvas *canvas = new TCanvas(canvas_name.str().c_str(), "c");// 1600, 900);
    canvas->cd();
