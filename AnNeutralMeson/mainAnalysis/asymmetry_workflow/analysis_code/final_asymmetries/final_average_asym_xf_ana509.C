...
void final_average_asym_xf_ana509(bool gpr_ratio = false,
                                  const std::string& special_tag = "")
{

  SetsPhenixStyle();
  std::string plots_folder = "xf_asymmetries" + special_tag + "/FINAL/";
  ...
  TFile *outputfile = new TFile(("full_average_xf" + special_tag + ratio_suffix + ".root").c_str(), "RECREATE"); 
  ...
  for (int iTrigger = 0; iTrigger < nTriggers; iTrigger++) {
    for (int iCrossingAngle = 0; iCrossingAngle < nCrossingAngles; iCrossingAngle++) {
      for (int iParticle = 0; iParticle < nParticles; iParticle++) {
        std::string inputfilename = inputfolder + "/analysis_asymmetry_xf_complete_ana509_01312026" + special_tag + "_" + trigger_labels[iTrigger] + pt_thresholds[iParticle] + "_" + crossing_angles[iCrossingAngle] + "_averaged_fitted" + ratio_suffix + ".root";
        ...
        for (int iBeam = 0; iBeam < nBeams; iBeam++) {
          for (int iMethod = 0; iMethod < nMethods; iMethod++) {
            ...
            // if (iMethod == 0) {
            //   std::cout << "xF: (iP, iT, iC, iB) = " << iParticle << ", " << iTrigger << ", " << iCrossingAngle << ", " << iBeam << ") = " << std::endl;
            //   graph_AN[iTrigger][iCrossingAngle][iBeam][iParticle][iMethod]->Print();
            // }
          }
        }
      }
  ...
  for (int iP = 0; iP < nParticles; iP++) {
    std::stringstream graphname;
    graphname << "graph_AN_xf_peak_" << particles[iP];
    ...
    shift_graph(graph_AN_final[iP][1], 0.002);
    ...
    double y_bound = GetAbsMaxInRange(graph_AN_final[iP][1], -0.3, 0.3);
  ...
  for (int iP = 0; iP < nParticles; iP++) {
    std::stringstream graphname;
    graphname << "graph_AN_xf_peak_beams_" << particles[iP];
    std::stringstream canvas_name;
    canvas_name << "canvas_" <<  graphname.str();
    TCanvas *canvas = new TCanvas(canvas_name.str().c_str(), "c");// 1600, 900);
    canvas->cd();
    ...
    shift_graph(graph_AN_triggerA_angleA[1][iP][0], 0.002);
    ...
    double y_bound = GetAbsMaxInRange(graph_AN_triggerA_angleA[1][iP][0], -0.3, 0.3);
    mg->SetMinimum(-2 * y_bound);
    mg->SetMaximum(2 * y_bound);
    ...
    // Compute the t-test
    {
      ...
      t_test->Draw("AP");
      y_bound = GetAbsMaxInRangeSimple(t_test, -0.3, 0.3);
  ...
  for (int iP = 0; iP < nParticles; iP++) {
    std::stringstream graphname;
    graphname << "graph_AN_xf_peak_angles_" << particles[iP];
    std::stringstream canvas_name;
    canvas_name << "canvas_" <<  graphname.str();
    TCanvas *canvas = new TCanvas(canvas_name.str().c_str(), "c");// 1600, 900);
    canvas->cd();
    ...
    double y_bound = GetAbsMaxInRange(graph_AN_triggerA_beamA[1][iP][0], -0.3, 0.3);
    mg->SetMinimum(-2 * y_bound);
    mg->SetMaximum(2 * y_bound);
  ...
  for (int iP = 0; iP < nParticles; iP++) {
    std::stringstream graphname;
    graphname << "graph_AN_xf_peak_triggers_" << particles[iP];
    std::stringstream canvas_name;
    canvas_name << "canvas_" <<  graphname.str();
    TCanvas *canvas = new TCanvas(canvas_name.str().c_str(), "c");// 1600, 900);
    canvas->cd();
    ...
    double y_bound = GetAbsMaxInRange(graph_AN_angleA_beamA[1][iP][0], -0.3, 0.3);
    TGraphErrors *graph_mbd_trimmed = nullptr;
    //trim_graph(graph_AN_angleA_beamA[0][iP][0], graph_mbd_trimmed, 1.5 * y_bound);
    cut_graph(graph_AN_angleA_beamA[0][iP][0], graph_mbd_trimmed, 2, 5);
