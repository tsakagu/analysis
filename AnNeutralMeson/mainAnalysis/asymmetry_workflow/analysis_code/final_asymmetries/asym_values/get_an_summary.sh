...
for first_suffix in "${arr_suffixes[@]}"; do
    ...
    paste -d'&' asym_stat_${suffix}.txt asym_sys_tot_${suffix}.txt asym_sys_calc_${suffix}.txt asym_sys_fit_${suffix}.txt asym_sys_shuffle_${suffix}.txt > asym_summary_${suffix}.txt
done
