...
void CreateSamePhenixPlot()
{
  std::string plots_folder_pt = "pt_asymmetries/PHENIX_ACCEPTANCE/";
  ...
  for (int iP = 0; iP < nParticles; iP++) {
    ...
    inputfilename << inputfolder << "asym_summary_" << particles[iP] << "_pT_phenix" << ".txt";
    ...
    parse_phenix_file(th_filename.str(), x_ph, y_ph, estat_ph, esyst_ph, exsyst_ph, 0.12);
    ...
    double y_bound = 0;
    if (iP == 0){
      y_bound = GetAbsMaxInRange(gStat, 2.0, 10);
    } else {
      y_bound = GetAbsMaxInRange(gStat, 2.0, 12);
    }
    ...
    if (iP == 0) gStat->GetXaxis()->SetLimits(2.0, 20.0);
    ...
    legend->AddEntry(gStat, "sPHENIX 2024, |#eta| < 0.35");
    ...
    latex.DrawLatex(0.67, 0.87, "#font[72]{sPHENIX} Internal");
    latex.DrawLatex(0.67, 0.77, "p^{#uparrow}+p #sqrt{s} = 200 GeV");
    latex.SetTextSize(0.03);
    latex.DrawLatex(0.64, 0.7, "7% polarization scale uncertainty not shown");
    latex.SetTextSize(0.05);

    if (iP == 0) {
      latex.DrawLatex(0.45, 0.85, "p^{#uparrow}+p #rightarrow #pi^{0} X");
    } else if (iP == 1) {
      latex.DrawLatex(0.45, 0.85, "p^{#uparrow}+p #rightarrow #eta X");
    }
