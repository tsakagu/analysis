...
int Fun4All_DST_SiliconCaloMatch(
   const int nEvents = 100, //0,
   const string in_dst = "/sphenix/tg/tg01/commissioning/INTT/work/mahiro/SIliconCalo/MC/dst_jpsi/condor/"
                         "DST_SiliconOnly_single_jpsi_vtxfixed_Caloevents_reconstructedInfo_0.root",
   const string out_root = "ana_0.root",
   const string out_dst = "DST_0.root",
...
)
{
  ...
  Fun4AllDstOutputManager *out = new Fun4AllDstOutputManager("DSTOUT", out_dst.c_str());
  se->registerOutputManager(out);


  //Tracking_QA();
  
  auto sicalo = new SiliconCaloMatching("SiliconCaloMatching");
  se->registerSubsystem(sicalo);
