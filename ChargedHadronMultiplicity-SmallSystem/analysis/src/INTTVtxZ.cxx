...
int main(int argc, char *argv[])
{
    ...
    float dPhi_cut = TString(argv[3]).Atof();                           // Example: 5 degree (0.0872664626 rad)
    float dca_cut = TString(argv[4]).Atof();                            // Example: 0.5cm
    ...
    // get the beamspot
    std::vector<float> beamspot = getBeamspot(bsresfilepath.Data(), "vertex");
    ...
    t->SetBranchAddress("mbdVertex", &mbdVertex);
    ...
    t->SetBranchAddress("cluster_ladderPhiId", &ClusLadderPhiId);
    ...
    for (int ev = 0; ev < NevtToRun; ev++)
    {
        ...
        // get the mbd and silicon vertex for crossing 0 (if not found, set to -9999)
        vtxdata.mbd_z_vtx = (mbdVertex->size() > 0) ? mbdVertex->at(0) : -9999.0;
        vtxdata.SiliconVertexZ_crossing0 = -9999.0;
        auto it = std::find(trackerVertexCrossing->begin(), trackerVertexCrossing->end(), 0);
        if (it != trackerVertexCrossing->end())
        {
            int index = std::distance(trackerVertexCrossing->begin(), it);
            vtxdata.SiliconVertexZ_crossing0 = trackerVertexZ->at(index);
        }
        vtxdata.firedTrig12_vtxle10cm = false;
        if (IsData && firedTriggers->size() > 0 && std::find(firedTriggers->begin(), firedTriggers->end(), 12) != firedTriggers->end())
        {
            vtxdata.firedTrig12_vtxle10cm = true;
        }

        cout << "event=" << event << ", GL1 BCO=" << gl1bco << ", total number of clusters=" << ClusLayer->size() << endl;
        ...
        {
            // for this study only look at time bucket ID = 0
        ...
        // Vertex quality
        bool isValidMBDZvtx = (vtxdata.mbd_z_vtx != vtxdata.mbd_z_vtx) ? false : true;
        bool isGoodINTTGausWidth = (sigma >= 3.0 && sigma <= 8.0) ? true : false;
        bool isMBDINTTZvtxMatch = (IsData && (mean - vtxdata.mbd_z_vtx > -5. && mean - vtxdata.mbd_z_vtx < 3.)) || (!IsData && fabs(mean - vtxdata.mbd_z_vtx) < 4.);
