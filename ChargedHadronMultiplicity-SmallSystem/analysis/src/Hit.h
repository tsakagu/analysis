...
class Hit : public TObject
{
  public:
    ...
    void SetEdgeFromZId(int);
    void SetMatchedG4P(int, int, int, int, int);
    void SetMatchedTkl();
    bool IsMatchedTkl();
    // a function to rotate the hit phi by pi, used for combinatoric background estimation; pass reference to the hit to be rotated, 
    // and it updates the hit's position and recalculate its eta, phi, R accordingly (vertex is unchanged)
    void RotatePhiByPi();
    void Print();
...
void Hit::SetEdgeFromZId(int zId)
{
    float edge = (zId == 0 || zId == 2) ? 0.8 : 0.5;
    _edge = make_pair(vechit.Z() - edge, vechit.Z() + edge);
}
...
void Hit::RotatePhiByPi()
{
    // rotate by phi -> phi + pi
    // this should change all coordinates (x,y,z, eta, phi) consistently
    _x = -vechit.X();
    _y = -vechit.Y();
    _z = vechit.Z(); // z is unchanged in this rotation
    vechit.SetXYZ(_x, _y, _z);
    vecrel = vechit - vecvtx;
    _eta = vecrel.Eta(); // recalculate eta, phi, R after rotation
    _phi = vecrel.Phi();
    _R = vecrel.Mag();
}
