...
int main(int argc, char *argv[])
{
    ...
    TString idxstr = (IsData) ? "gl1bco" : "event";

    TRandom3 *rnd = new TRandom3(0); // Random number generator

    TrackletData tkldata = {};

    std::map<int, vertex> EvtVtxMap_event = EvtVtx_map_event(EvtVtx_map_filename.Data());          // use with simulation
    std::map<uint64_t, vertex> EvtVtxMap_inttbco = EvtVtx_map_inttbco(EvtVtx_map_filename.Data()); // use with data
    ...
    for (int i = 0; i < NevtToRun_; i++)
    {
        ...
        cout << "i = " << i << " event = " << event << " GL1 BCO = " << gl1bco << " NevtToRun_ = " << NevtToRun_ << " local = " << local << " InttBco_IsToBeRemoved = " << InttBco_IsToBeRemoved << endl;

        // Discard events with a small mutual BCO difference between adjacent events
        if (InttBco_IsToBeRemoved)
            continue;

        vertex PV = (IsData) ? EvtVtxMap_inttbco[gl1bco] : EvtVtxMap_event[event];

        // skip event if no silicon vertex with crossing 0 is found
        if (PV.silicon_z_crossing0 < -9990.0)
            continue;

        std::cout << "PV info: beamspot_x = " << PV.beamspot_x << "; beamspot_y = " << PV.beamspot_y << "; mbd_z = " << PV.mbd_z << "; silicon_z_crossing0 = " << PV.silicon_z_crossing0 << std::endl;

        tkldata.event = event;
        // tkldata.INTT_BCO = INTT_BCO;
        tkldata.gl1bco = gl1bco;
        tkldata.bcotr = bcotr;
        tkldata.PV_x = PV.beamspot_x;
        tkldata.PV_y = PV.beamspot_y;
        float PVz = (userandomzvtx) ? rnd->Uniform(-30, 30) : PV.mbd_z; //! For this study, we use mbd_z as reconstructed PV_z
        tkldata.PV_z = PVz;
        ...
        else
        {
            if (IsData)
                ...
                tkldata.vtxzwei = hM_vtxzweight->GetBinContent(hM_vtxzweight->FindBin(PV.mbd_z));
        }
        ...
        tkldata.MbdZvtxle10cm = PV.mbd_z >= -10.0 && PV.mbd_z <= 10.0;
        tkldata.validMbdVtx = (PV.mbd_z > -9999.0) ? true : false;
        tkldata.InttBco_IsToBeRemoved = false; // For this study we don't remove any BCO

        // the total number of silicon seeds with crossing 0 by counting number of silseeds with crossing 0
        tkldata.Nsiliconseeds = std::count(trackerVertexCrossing->begin(), trackerVertexCrossing->end(), 0);

        // get the index of vertex with crossing = 0
        auto vtx_it = std::find(trackerVertexCrossing->begin(), trackerVertexCrossing->end(), 0);
        if (vtx_it != trackerVertexCrossing->end())
        {
            int vtx_index = std::distance(trackerVertexCrossing->begin(), vtx_it);
            tkldata.Nsiliconseeds_vtx = trackerVertexNTracks->at(vtx_index);
        }
        else
        {
            tkldata.Nsiliconseeds_vtx = 0;
        }

        // Centrality
        // tkldata.centrality_mbd = centrality_mbd;
        tkldata.mbd_z_vtx = PV.mbd_z;
        ...
        cout << "Number of clusters with time bucket ID = 0: " << std::count(cluster_timeBucketID->begin(), cluster_timeBucketID->end(), 0) << endl;
        ...
        for (size_t ihit = 0; ihit < ClusLayer->size(); ihit++)
        {
            ...
            // skip clusters with time bucket ID != 0
            if (cluster_timeBucketID->at(ihit) != 0)
                continue;

            int layer = (ClusLayer->at(ihit) == 3 || ClusLayer->at(ihit) == 4) ? 0 : 1;
            Hit *hit = new Hit(ClusX->at(ihit), ClusY->at(ihit), ClusZ->at(ihit), PV.beamspot_x, PV.beamspot_y, PVz, layer, ClusPhiSize->at(ihit), 1); // set cluster ADC to 1 for this study
        ...
        if (Nrandclus > 0)
        {
            for (int irand = 0; irand < Nrandclus; irand++)
            {
                ...
                Hit *randhit_inner = RandomHit(PV.beamspot_x, PV.beamspot_y, PVz, 0);
                Hit *randhit_outer = RandomHit(PV.beamspot_x, PV.beamspot_y, PVz, 1);
