...
with open(sys.argv[1],'r') as f:
    for line in f:
        ...
        if(parts[0] =="Raw:"):
            ...
            x1.append(int(parts[3]))
            y1.append(int(SumCharge))
            x2.append(int(parts[4]))
            y2.append(int(SumSamp))

            h1.Fill(int(nline))
            nline=0

            if(int(SumSamp)!=int(parts[4])):
                error+=1

            if(float(parts[4])>0 and float(SumSamp)>0):
                x3.append(float(parts[3])/float(parts[4]))
                y3.append(float(SumCharge)/float(SumSamp))
            SumCharge=0
            SumSamp=0
...
gr1.SetTitle("DC: FEE firmware vs 100% streaming, run 72443, sector 23, Fee 11, ch 201")
