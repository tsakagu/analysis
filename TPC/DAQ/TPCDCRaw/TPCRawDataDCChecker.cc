...
TPCRawDataDCChecker::TPCRawDataDCChecker(const std::string &name)
  : SubsysReco("TPCRawDataDCChecker")
  , m_fname(name)
{
...
//____________________________________________________________________________..
int TPCRawDataDCChecker::InitRun(PHCompositeNode *)
{
//  sectorNum = m_fname;
...
//  if(stoi(sectorNum) > 11) side = 1;
  ...
  m_SampleTree->Branch("NSampDig", &m_NSampDig, "NSampDig/s");
  m_SampleTree->Branch("SumDig", &m_SumDig, "SumDig/s");
  ...
  m_SampleTree_DC = new TTree("SampleTreeDC", "Each entry is one dc packet");

  m_SampleTree_DC->Branch("packet", &m_packet, "packet/I");
  m_SampleTree_DC->Branch("frame", &m_frame, "frame/I");
  m_SampleTree_DC->Branch("nCurrentInFrame", &m_nCurrentInFrame, "nCurrentInFrame/I");
  m_SampleTree_DC->Branch("maxFEECount", &m_maxFEECount, "maxFEECount/I");

  m_SampleTree_DC->Branch("DCValues", &m_DCValues[0], "DCValues[8]/I");
  m_SampleTree_DC->Branch("DCSamples", &m_DCSamples[0], "DCSamples[8]/I");
  m_SampleTree_DC->Branch("fee", &m_fee_DC, "fee/I");
  m_SampleTree_DC->Branch("sampaAddress", &m_sampaAddress_DC, "sampaAddress/I");
  m_SampleTree_DC->Branch("sampaMAXChannel", &m_sampaMAXChannel_DC, "sampaMAXChannel/I");
  m_SampleTree_DC->Branch("BCO", &m_BCO_DC, "BCO/I");
  m_SampleTree_DC->Branch("checksum", &m_checksum_DC, "checksum/I");
  m_SampleTree_DC->Branch("checksumError", &m_checksumError_DC, "checksumError/I");
  m_SampleTree_DC->Branch("Lv1TaggerBCO", &m_Lv1TaggerBCO, "Lv1TaggerBCO/l");
...
//____________________________________________________________________________..
int TPCRawDataDCChecker::process_event(PHCompositeNode *topNode)
{
  ...
  for (int packet : m_packets)
  {
    ...
    m_nCurrentInFrame = p->iValue(0, "NR_DC");
    ...
    for (int wf = 0; wf < m_nWaveormInFrame; wf++)
    {
      ...
      for (int s = 0; s < m_nSamples; s++)
      {
        ...
        if(m_adcSamples[s]<4000){
           m_SumDig += m_adcSamples[s];
           m_NSampDig ++;
        }
...
//          double phi = (stoi(sectorNum)<12? -M.getPhi(feeM, m_Channel) : M.getPhi(feeM, m_Channel) ) + (stoi(sectorNum) - side*12. )* M_PI / 6. ;
    ...
    // Here, digital current packet
    for (int wf = 0; wf < m_nCurrentInFrame; wf++)
    {
      m_BCO_DC = p->iValue(wf, "BCO_DC");
      m_fee_DC = p->iValue(wf, "FEE_DC");

      m_sampaAddress_DC = p->iValue(wf, "SAMPAADDRESS_DC");
      m_sampaMAXChannel_DC = p->iValue(wf, "SAMPAMAXCHANNEL_DC");
      m_Channel_DC = p->iValue(wf, "CHANNEL_DC");
      m_checksum_DC = p->iValue(wf, "CHECKSUM_DC");
      m_checksumError_DC = p->iValue(wf, "CHECKSUMERROR_DC");

      for ( int i=0;i<8;i++){
        m_DCValues[i] = p->iValue(wf, i,"DC_VAL");
        m_DCSamples[i] = p->iValue(wf, i,"DC_NSAMPLES");
      }
      m_SampleTree_DC->Fill();
    }
...
//____________________________________________________________________________..
int TPCRawDataDCChecker::End(PHCompositeNode * /*topNode*/)
{
