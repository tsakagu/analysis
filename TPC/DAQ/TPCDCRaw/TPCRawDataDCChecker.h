...
//  -*- C++ -*-.
#ifndef TPCRawDataDCChecker_H
#define TPCRawDataDCChecker_H
...
class TPCRawDataDCChecker : public SubsysReco
{
 public:
  explicit TPCRawDataDCChecker(const std::string &fname = "TPCRawDataDCChecker.root");

  ~TPCRawDataDCChecker() override {}
 ...
 private:
  ...
  // Digital current
  int m_nCurrentInFrame = 0;
  int m_fee_DC = 0;
  int m_sampaAddress_DC = 0;
  int m_sampaMAXChannel_DC = 0;
  int m_Channel_DC = 0;
  int m_BCO_DC = 0;
  int m_checksum_DC = 0;
  int m_checksumError_DC = 0;

  int m_DCValues[8]={0,0,0,0,0,0,0,0};
  int m_DCSamples[8]={0,0,0,0,0,0,0,0};
